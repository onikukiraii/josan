...
| external_night_count | INTEGER | 他院夜勤回数（デフォルト0、0〜5） |
| position | INTEGER | 表示順（昇順） |
...
| date | DATE | 希望休の日付 |
| request_type | ENUM | 希望種別（day_off / paid_leave / day_shift_request / night_shift_request） |
| created_at | TIMESTAMP | |
...
| day_off | 公休 |
| paid_leave | 有給 |
| day_shift_request | 日勤希望 |
| night_shift_request | 夜勤希望 |
